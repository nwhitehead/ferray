// Integration tests for ferrum-core-macros
//
// These tests exercise the proc macros (FerrumRecord, s!, promoted_type!)
// from the consumer perspective — i.e., they use the macros via ferrum-core's
// public re-exports, just as a downstream crate would.

use ferrum_core::dtype::{DType, SliceInfoElem};
use ferrum_core::record::FerrumRecord;
...
#[repr(C)]
#[derive(Clone, Debug, ferrum_core::FerrumRecord)]
struct Point {
    x: f64,
    y: f64,
}
...
fn s_macro_range_with_step() {
    let slices = ferrum_core::s![1..5;2];
    assert_eq!(slices.len(), 1);
    assert_eq!(
        slices[0],
        SliceInfoElem::Slice {
...
fn s_macro_multi_axis() {
    let slices = ferrum_core::s![0..3, 2];
...
fn promoted_type_f32_f64() {
    ...
    let _: ferrum_core::promoted_type!(f32, f64) = 1.0f64;
}

#[test]
fn promoted_type_i32_f32() {
    // i32 + f32 -> f64 (because i32 needs 53-bit mantissa)
    let _: ferrum_core::promoted_type!(i32, f32) = 1.0f64;
}

#[test]
fn promoted_type_u8_i8() {
    // u8 + i8 -> i16
    let _: ferrum_core::promoted_type!(u8, i8) = 1i16;
}

#[test]
fn promoted_type_same() {
    let _: ferrum_core::promoted_type!(f64, f64) = 1.0f64;
    let _: ferrum_core::promoted_type!(i32, i32) = 1i32;
}

#[test]
fn promoted_type_bool_int() {
    let _: ferrum_core::promoted_type!(bool, i32) = 1i32;
}

#[test]
fn promoted_type_complex() {
    let _: ferrum_core::promoted_type!(Complex<f32>, f64) =
